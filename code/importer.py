...
os.environ['DJANGO_SETTINGS_MODULE'] = 'simpleat.settings'
import django
django.setup()
...
with open(filepath + 'course-data.csv') as csvfile:
    ...
    obj_create = []
    for num, row in enumerate(reader):
        # Skip existing Course IDs
        if Course.objects.filter(pk=num + 1).exists():
            continue

        try:
            teacher = User.objects.get(pk=int(row['teacher']))
        except User.DoesNotExist:
            print(f"⚠️ Skipping row {num+1}: User with ID {row['teacher']} does not exist.")
            continue
